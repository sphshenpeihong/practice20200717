...
public class UserMgrCtl {
    ...
    public ResponseEntity<Response> test1() throws Exception{
        //初始化分页对象存放到ThreadLocal里面的Map中
        PageHelper.startPage(6, 3, "id asc");
    ...
    public ResponseEntity<Response> test22() throws Exception{
        //初始化分页对象存放到ThreadLocal里面的Map中
        PageHelper.startPage(-2, 10, "id asc");
        //使用了分页插件的话，那么SQL就不能再去重复写Limit了，因为Mybatis底层也是通过获取ThreadLocal的值
        List<QyUserPO> list = userService.getAllUser();
        //利用PageInfo的构造方法去get到相关值
        //因为使用了分页插件，所以实际查询完毕的实例类型是Page，继承了ArrayList
        ...
        //故重新定义一个PageInfo对象去接收相关的值呢。
        PageInfo<QyUserPO> pageInfo = new PageInfo<>(list, 3);
        Pager pager = Pager.formatRespPage(pageInfo);
        return Response.ok(pager);
    }

    @RequestMapping("/test23.do")
    public ResponseEntity<Response> test23() throws Exception{
        //初始化分页对象存放到ThreadLocal里面的Map中
        ReqPager req = WebUtil.getReqPager();
        PageHelper.startPage(req.getCurrentPage(), req.getPageSize(), "id asc");
        //使用了分页插件的话，那么SQL就不能再去重复写Limit了，因为Mybatis底层也是通过获取ThreadLocal的值
        List<QyUserPO> list = userService.getAllUser();
        //利用PageInfo的构造方法去get到相关值
        //因为使用了分页插件，所以实际查询完毕的实例类型是Page，继承了ArrayList
